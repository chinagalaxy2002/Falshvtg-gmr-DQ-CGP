...
def compute_average_precision_detection(ground_truth,
    ...
    predictions data frames. Also computes False Alarm Rate (FAR) and
    ...
    Returns:
        Tuple: (ap, far, md)
            - ap (np.ndarray): Average precision score for each IoU threshold.
            - far (np.ndarray): False alarm rate for each IoU threshold.
            - md (np.ndarray): Miss detection rate for each IoU threshold.
    """
    num_thresholds = len(tiou_thresholds)
    num_gts = len(ground_truth)
    num_preds = len(prediction)
    ap = np.zeros(num_thresholds)
    far = np.zeros((num_thresholds, num_preds))
    md = np.zeros((num_thresholds, num_preds))
    ...
    for t_idx in range(len(tiou_thresholds)):
        ...
        for idx in range(num_preds):
            total_fp = fp_cumsum[t_idx, idx]
            total_tp = tp_cumsum[t_idx, idx]
            total_fn = num_gts - total_tp

            far[t_idx, idx] = total_fp / (total_fp + num_gts) # (thresholds, #predictions)
            md[t_idx, idx] = total_fn / num_positive

    far = np.mean(far, axis=1) # Average over all predictions, same as "a" in ap
