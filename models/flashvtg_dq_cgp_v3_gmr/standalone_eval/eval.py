...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14)):
    ...
    for d in ground_truth:
        ...
        cur_qid = d["qid"]
        # Negative query (empty GT): skip for MR-R1/mIoU computation.
        if len(cur_gt_windows) == 0:
            continue
        if cur_qid not in pred_qid2window:
            continue
        cur_max_iou_idx = 0
    ...
    qids = list(gt_qid2window.keys())
    if len(qids) == 0:
        iou_thd2recall_at_one = {str(thd): 0.0 for thd in iou_thds}
        return iou_thd2recall_at_one, 0.0
...
def eval_moment_retrieval(submission, ground_truth, verbose=True, full_only=False):
    if full_only:
        length_ranges = [[0, 150]]
        range_names = ["full"]
    else:
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        # GMR setting may include negatives with empty relevant_windows.
        # MR metrics are defined on positives only, so filter empties out.
        pos_qids = {d["qid"] for d in _ground_truth if len(d.get("relevant_windows", [])) > 0}
        _ground_truth = [d for d in _ground_truth if d["qid"] in pos_qids]
        _submission = [d for d in _submission if d["qid"] in pos_qids]
...
def eval_submission(submission, ground_truth, verbose=True, match_number=True, full_only=False, mr_only=False):
    """
    Args:
    ...
    if "pred_relevant_windows" in submission[0]:
        moment_ret_scores = eval_moment_retrieval(
            submission, ground_truth, verbose=verbose, full_only=full_only)
        ...
        if not full_only:
            moment_ret_scores_brief.update({
    ...
    if (not mr_only) and ("pred_saliency_scores" in submission[0]):
...
def eval_main():
    ...
    parser.add_argument("--full_only", action="store_true",
                        help="Only evaluate full-range MR metrics.")
    parser.add_argument("--mr_only", action="store_true",
                        help="Only evaluate MR metrics. Skip HL metrics even if submission has saliency scores.")
    args = parser.parse_args()

    verbose = not args.not_verbose
    submission = load_jsonl(args.submission_path)
    gt = load_jsonl(args.gt_path)
    results = eval_submission(submission, gt, verbose=verbose, full_only=args.full_only, mr_only=args.mr_only)
    if verbose:
