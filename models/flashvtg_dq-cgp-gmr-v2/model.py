...
from .dq_cgp import FlashPointDQCGP
...
class FlashVTGDQCGP(nn.Module):
    """FlashVTG-GMR with candidate-wise DQ-CGP point refinement."""
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, n_input_proj, input_dropout, txt_dim, vid_dim, aud_dim=0, use_txt_pos=False,
        ...
        # DQ-CGP is deliberately added after every baseline module above.  A
        # baseline state_dict can therefore initialize all common parameters,
        # and beta=0 leaves the baseline forward path exactly unchanged.
        self.use_dq_cgp = bool(_get_option(args, "use_dq_cgp", True))
        if self.use_dq_cgp:
            self.dq_cgp = FlashPointDQCGP(
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, vid, qid, targets=None):
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            if dq_cgp_output is not None:
                ...
                output["dq_cgp_update_gate"] = dq_cgp_output.update_gate
...
class SetCriterion(nn.Module):
    ...
    def loss_dq_cgp(self, data):
        """Supervise point-wise temporal binding and compositional routing."""
        ...
        if reference is None:
            raise KeyError("DQ-CGP loss requires out_class or pred_exist_logits")
        zero = reference.sum() * 0.0
        if data.get("boundary") is None or not required.issubset(data):
            return {
        ...
        update_gate = data["dq_cgp_update_gate"]
        ...
        fps = data.get("fps")
        if fps is None:
            raise KeyError("DQ-CGP binding loss requires fps")
        ...
        for batch_index in range(attention.shape[0]):
            ...
            positive = positive & candidate_mask[batch_index]
            positive_indices = torch.where(positive)[0]
            background_indices = torch.where(
                candidate_mask[batch_index] & ~positive
            )[0]
            if positive_indices.numel() > 0:
                gate_terms.append(
                    -update_gate[batch_index, positive_indices]
                    .clamp_min(eps)
                    .log()
                    .mean()
                )
            if background_indices.numel() > 0:
                gate_terms.append(
                    -(1.0 - update_gate[batch_index, background_indices])
                    .clamp_min(eps)
                    .log()
                    .mean()
                )
            ...
            finite_gt = torch.isfinite(gt_boundary).all(dim=-1)
            finite_gt = finite_gt & (gt_boundary[:, 1] > gt_boundary[:, 0])
            valid_gt = gt_boundary[finite_gt]
            selected_gt = valid_gt[matched_gt[positive_indices]]
            overlap = (
                (frame_index.unsqueeze(0) < selected_gt[:, 1:])
                & ((frame_index + 1.0).unsqueeze(0) > selected_gt[:, :1])
            )
            overlap = overlap & video_mask[batch_index].unsqueeze(0)
            matched_attention = attention[batch_index, positive_indices]
            target_mass = (
                matched_attention * overlap.to(matched_attention.dtype)
            ).sum(dim=-1)
            binding_terms.append(-target_mass.clamp_min(eps).log())
            matched_routes.append(basis_weights[batch_index, positive_indices])

        gate_loss = (
            torch.stack(gate_terms).mean()
            if gate_terms
            else update_gate.sum() * 0.0
        )
        if not binding_terms:
            return {
                "loss_dq_cgp_bind": attention.sum() * 0.0,
                "loss_dq_cgp_route": basis_weights.sum() * 0.0,
                "loss_dq_cgp_gate": gate_loss,
            }
    ...
    @staticmethod
    def loss_dq_cgp_null_gate(outputs, exist_labels):
        """Keep every candidate close to identity for a null query-video pair."""

        gate = outputs.get("dq_cgp_update_gate")
        candidate_mask = outputs.get("dq_cgp_candidate_mask")
        if gate is None or candidate_mask is None or exist_labels is None:
            reference = outputs.get("out_class", outputs.get("pred_exist_logits"))
            return reference.sum() * 0.0
        negative = exist_labels.view(-1) <= 0.5
        valid = candidate_mask.bool() & negative.unsqueeze(1)
        if not bool(valid.any()):
            return gate.sum() * 0.0
        eps = torch.finfo(gate.dtype).eps
        return -(1.0 - gate[valid]).clamp_min(eps).log().mean()
    ...
    def forward(self, batch, outputs, targets):
        ...
        if bool(_get_option(self.args, "use_dq_cgp", True)):
            null_gate_loss = self.loss_dq_cgp_null_gate(outputs, exist_labels)
            if "loss_dq_cgp_gate" in losses:
                has_negative = exist_labels is not None and bool(
                    (exist_labels.view(-1) <= 0.5).any()
                )
                if has_negative:
                    losses["loss_dq_cgp_gate"] = 0.5 * (
                        losses["loss_dq_cgp_gate"] + null_gate_loss
                    )
            else:
                losses["loss_dq_cgp_gate"] = null_gate_loss
...
def build_model1(args):
    ...
    if bool(_get_option(args, "use_dq_cgp", True)):
        ...
        weight_dict["loss_dq_cgp_gate"] = float(
            _get_option(args, "dq_cgp_gate_loss_coef", 0.05)
        )
